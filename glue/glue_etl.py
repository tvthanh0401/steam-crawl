...
def load_data_reviews(reviews, new_sale_games, secrets):
...
def load_data_from_dataframe_to_dynamodb(df_reviews, df_deal, df_game, new_sale_games, secrets):
    ...
    reviews_process = Process(target=load_data_reviews,
                              args=(df_reviews, new_sale_games, secrets))
...
def main():
    ...
    previous_day_str = f"{previous.day}_{previous.month}_{previous.year}.json"
    previous_deal = "deal/deal" + previous_day_str
    previous_link = "link/link" + previous_day_str
    previous_reviews = "reviews/reviews" + previous_day_str
    ...
    prev_links = get_data_from_s3(bucket_name, previous_link)
    prev_deals = get_data_from_s3(bucket_name, previous_deal)
    prev_reviews = get_data_from_s3(bucket_name, previous_reviews)


    df_deal, df_link, df_reviews = extract_data_from_s3(deals, links, reviews)

    prev_df_deal, prev_df_link, prev_df_reviews = extract_data_from_s3(prev_deals, prev_links, prev_reviews)

    df_reviews, df_deal, df_game = transform_data_from_extract(
        df_reviews, df_link, df_deal)

    prev_df_reviews, prev_df_deal, prev_df_game = transform_data_from_extract(prev_df_reviews, 
    prev_df_link, prev_df_deal)

    
    already_in_db = set()
    new_sale_games = set()

    for _, deal in prev_df_deal.iterrows():
        app_id = link_to_id(deal['link'])
        already_in_db.add((app_id, deal['timestamp']))
    
    for _, deal in df_deal.iterrows():
        app_id = link_to_id(deal['link'])
        if (app_id, deal['timestamp']) not in already_in_db:
            new_sale_games.add(str(app_id))
    ...
    load_data_from_dataframe_to_dynamodb(df_reviews, df_deal, df_game, new_sale_games, secrets)
